'use client';
import React from 'react';

interface Color {
  name: string;
  hue: number;
  variants: [number, number, number][];
}

const colors: Color[] = [
  {
    name: 'rose',
    hue: 16.439,
    variants: [
      [0.969, 0.015, 12.422],
      [0.941, 0.03, 12.58],
      [0.892, 0.058, 10.001],
      [0.81, 0.117, 11.638],
      [0.712, 0.194, 13.428],
      [0.645, 0.246, 16.439],
      [0.586, 0.253, 17.585],
      [0.514, 0.222, 16.935],
      [0.455, 0.188, 13.697],
      [0.41, 0.159, 10.272],
      [0.271, 0.105, 12.094],
    ],
  },
  {
    name: 'red',
    hue: 25.331,
    variants: [
      [0.971, 0.013, 17.38],
      [0.936, 0.032, 17.717],
      [0.885, 0.062, 18.334],
      [0.808, 0.114, 19.571],
      [0.704, 0.191, 22.216],
      [0.637, 0.237, 25.331],
      [0.577, 0.245, 27.325],
      [0.505, 0.213, 27.518],
      [0.444, 0.177, 26.899],
      [0.396, 0.141, 25.723],
      [0.258, 0.092, 26.042],
    ],
  },
  {
    name: 'orange',
    hue: 47.604,
    variants: [
      [0.98, 0.016, 73.684],
      [0.954, 0.038, 75.164],
      [0.901, 0.076, 70.697],
      [0.837, 0.128, 66.29],
      [0.75, 0.183, 55.934],
      [0.705, 0.213, 47.604],
      [0.646, 0.222, 41.116],
      [0.553, 0.195, 38.402],
      [0.47, 0.157, 37.304],
      [0.408, 0.123, 38.172],
      [0.266, 0.079, 36.259],
    ],
  },
  {
    name: 'amber',
    hue: 70.08,
    variants: [
      [0.987, 0.022, 95.277],
      [0.962, 0.059, 95.617],
      [0.924, 0.12, 95.746],
      [0.879, 0.169, 91.605],
      [0.828, 0.189, 84.429],
      [0.769, 0.188, 70.08],
      [0.666, 0.179, 58.318],
      [0.555, 0.163, 48.998],
      [0.473, 0.137, 46.201],
      [0.414, 0.112, 45.904],
      [0.279, 0.077, 45.635],
    ],
  },
  {
    name: 'yellow',
    hue: 86.047,
    variants: [
      [0.987, 0.026, 102.212],
      [0.973, 0.071, 103.193],
      [0.945, 0.129, 101.54],
      [0.905, 0.182, 98.111],
      [0.852, 0.199, 91.936],
      [0.795, 0.184, 86.047],
      [0.681, 0.162, 75.834],
      [0.554, 0.135, 66.442],
      [0.476, 0.114, 61.907],
      [0.421, 0.095, 57.708],
      [0.286, 0.066, 53.813],
    ],
  },
  {
    name: 'lime',
    hue: 130.85,
    variants: [
      [0.986, 0.031, 120.757],
      [0.967, 0.067, 122.328],
      [0.938, 0.127, 124.321],
      [0.897, 0.196, 126.665],
      [0.841, 0.238, 128.85],
      [0.768, 0.233, 130.85],
      [0.648, 0.2, 131.684],
      [0.532, 0.157, 131.589],
      [0.453, 0.124, 130.933],
      [0.405, 0.101, 131.063],
      [0.274, 0.072, 132.109],
    ],
  },
  {
    name: 'green',
    hue: 149.579,
    variants: [
      [0.982, 0.018, 155.826],
      [0.962, 0.044, 156.743],
      [0.925, 0.084, 155.995],
      [0.871, 0.15, 154.449],
      [0.792, 0.209, 151.711],
      [0.723, 0.219, 149.579],
      [0.627, 0.194, 149.214],
      [0.527, 0.154, 150.069],
      [0.448, 0.119, 151.328],
      [0.393, 0.095, 152.535],
      [0.266, 0.065, 152.934],
    ],
  },
  {
    name: 'emerald',
    hue: 162.48,
    variants: [
      [0.979, 0.021, 166.113],
      [0.95, 0.052, 163.051],
      [0.905, 0.093, 164.15],
      [0.845, 0.143, 164.978],
      [0.765, 0.177, 163.223],
      [0.696, 0.17, 162.48],
      [0.596, 0.145, 163.225],
      [0.508, 0.118, 165.612],
      [0.432, 0.095, 166.913],
      [0.378, 0.077, 168.94],
      [0.262, 0.051, 172.552],
    ],
  },
  {
    name: 'teal',
    hue: 182.503,
    variants: [
      [0.984, 0.014, 180.72],
      [0.953, 0.051, 180.801],
      [0.91, 0.096, 180.426],
      [0.855, 0.138, 181.071],
      [0.777, 0.152, 181.912],
      [0.704, 0.14, 182.503],
      [0.6, 0.118, 184.704],
      [0.511, 0.096, 186.391],
      [0.437, 0.078, 188.216],
      [0.386, 0.063, 188.416],
      [0.277, 0.046, 192.524],
    ],
  },
  {
    name: 'cyan',
    hue: 215.221,
    variants: [
      [0.984, 0.019, 200.873],
      [0.956, 0.045, 203.388],
      [0.917, 0.08, 205.041],
      [0.865, 0.127, 207.078],
      [0.789, 0.154, 211.53],
      [0.715, 0.143, 215.221],
      [0.609, 0.126, 221.723],
      [0.52, 0.105, 223.128],
      [0.45, 0.085, 224.283],
      [0.398, 0.07, 227.392],
      [0.302, 0.056, 229.695],
    ],
  },
  {
    name: 'sky',
    hue: 237.323,
    variants: [
      [0.977, 0.013, 236.62],
      [0.951, 0.026, 236.824],
      [0.901, 0.058, 230.902],
      [0.828, 0.111, 230.318],
      [0.746, 0.16, 232.661],
      [0.685, 0.169, 237.323],
      [0.588, 0.158, 241.966],
      [0.5, 0.134, 242.749],
      [0.443, 0.11, 240.79],
      [0.391, 0.09, 240.876],
      [0.293, 0.066, 243.157],
    ],
  },
  {
    name: 'blue',
    hue: 259.815,
    variants: [
      [0.97, 0.014, 254.604],
      [0.932, 0.032, 255.585],
      [0.882, 0.059, 254.128],
      [0.809, 0.105, 251.813],
      [0.707, 0.165, 254.624],
      [0.623, 0.214, 259.815],
      [0.546, 0.245, 262.881],
      [0.488, 0.243, 264.376],
      [0.424, 0.199, 265.638],
      [0.379, 0.146, 265.522],
      [0.282, 0.091, 267.935],
    ],
  },
  {
    name: 'indigo',
    hue: 277.117,
    variants: [
      [0.962, 0.018, 272.314],
      [0.93, 0.034, 272.788],
      [0.87, 0.065, 274.039],
      [0.785, 0.115, 274.713],
      [0.673, 0.182, 276.935],
      [0.585, 0.233, 277.117],
      [0.511, 0.262, 276.966],
      [0.457, 0.24, 277.023],
      [0.398, 0.195, 277.366],
      [0.359, 0.144, 278.697],
      [0.257, 0.09, 281.288],
    ],
  },
  {
    name: 'violet',
    hue: 292.717,
    variants: [
      [0.969, 0.016, 293.756],
      [0.943, 0.029, 294.588],
      [0.894, 0.057, 293.283],
      [0.811, 0.111, 293.571],
      [0.702, 0.183, 293.541],
      [0.606, 0.25, 292.717],
      [0.541, 0.281, 293.009],
      [0.491, 0.27, 292.581],
      [0.432, 0.232, 292.759],
      [0.38, 0.189, 293.745],
      [0.283, 0.141, 291.089],
    ],
  },
  {
    name: 'purple',
    hue: 303.9,
    variants: [
      [0.977, 0.014, 308.299],
      [0.946, 0.033, 307.174],
      [0.902, 0.063, 306.703],
      [0.827, 0.119, 306.383],
      [0.714, 0.203, 305.504],
      [0.627, 0.265, 303.9],
      [0.558, 0.288, 302.321],
      [0.496, 0.265, 301.924],
      [0.438, 0.218, 303.724],
      [0.381, 0.176, 304.987],
      [0.291, 0.149, 302.717],
    ],
  },
  {
    name: 'fuchsia',
    hue: 322.15,
    variants: [
      [0.977, 0.017, 320.058],
      [0.952, 0.037, 318.852],
      [0.903, 0.076, 319.62],
      [0.833, 0.145, 321.434],
      [0.74, 0.238, 322.16],
      [0.667, 0.295, 322.15],
      [0.591, 0.293, 322.896],
      [0.518, 0.253, 323.949],
      [0.452, 0.211, 324.591],
      [0.401, 0.17, 325.612],
      [0.293, 0.136, 325.661],
    ],
  },
  {
    name: 'pink',
    hue: 354.308,
    variants: [
      [0.971, 0.014, 343.198],
      [0.948, 0.028, 342.258],
      [0.899, 0.061, 343.231],
      [0.823, 0.12, 346.018],
      [0.718, 0.202, 349.761],
      [0.656, 0.241, 354.308],
      [0.592, 0.249, 0.584],
      [0.525, 0.223, 3.958],
      [0.459, 0.187, 3.815],
      [0.408, 0.153, 2.432],
      [0.284, 0.109, 3.907],
    ],
  },
];

function getColor(hue: number): [Color, Color, Color] {
  let over = colors.findIndex((c) => c.hue >= hue);
  if (over === -1) over = 0;
  const under = over === 0 ? colors.length - 1 : over - 1;
  let overHue = colors[over].hue;
  if (overHue < hue) overHue += 360;
  let underHue = colors[under].hue;
  if (underHue > hue) underHue -= 360;
  const interpolate = (hue - overHue) / (overHue - underHue);

  return [
    {
      name: 'custom',
      hue,
      variants: colors[over].variants.map((v, i) => {
        return v.map((w, j) => {
          let underV = colors[under].variants[i][j];
          if (j === 2 && underV > w) underV -= 360;
          return w + interpolate * (w - underV);
        }) as [number, number, number];
      }),
    },
    colors[over],
    colors[under],
  ];
}

function writeColor(color: Color, over: Color, under: Color): void {
  const el = document.documentElement.style;
  const weight = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950];
  color.variants.forEach((v, i) => {
    el.setProperty(
      `--color-brand-${weight[i]}`,
      `oklch(${v[0]} ${v[1]} ${v[2]})`
    );
  });
  over.variants.forEach((v, i) => {
    el.setProperty(
      `--color-over-${weight[i]}`,
      `oklch(${v[0]} ${v[1]} ${v[2]})`
    );
  });
  under.variants.forEach((v, i) => {
    el.setProperty(
      `--color-under-${weight[i]}`,
      `oklch(${v[0]} ${v[1]} ${v[2]})`
    );
  });
}

export default function Home() {
  const [state, setState] = React.useState(180);

  const [custom, over, under] = [...getColor(state)];
  React.useEffect(() => {
    writeColor(custom, over, under);
  });

  return (
    <main className="flex flex-col h-dvh">
      <div className="flex">
        <input
          type="range"
          min="0"
          max="360"
          className="grow accent-brand-500"
          value={state}
          onChange={(event) => {
            setState(Number(event.target.value));
          }}
        ></input>
      </div>
      <div className="flex grow gap-2">
        <div className="flex grow flex-col basis-1/3">
          <div className="p-2 flex justify-between">
            <span>{under.name}</span>
            <span>{under.hue}</span>
          </div>
          <div className="grow bg-under-50"></div>
          <div className="grow bg-under-100"></div>
          <div className="grow bg-under-200"></div>
          <div className="grow bg-under-300"></div>
          <div className="grow bg-under-400"></div>
          <div className="grow bg-under-500"></div>
          <div className="grow bg-under-600"></div>
          <div className="grow bg-under-700"></div>
          <div className="grow bg-under-800"></div>
          <div className="grow bg-under-900"></div>
          <div className="grow bg-under-950"></div>
        </div>
        <div className="flex grow flex-col basis-1/3">
          <div className="p-2 flex justify-between">
            <span>custom</span>
            <span>{state}</span>
          </div>
          <div className="grow bg-brand-50"></div>
          <div className="grow bg-brand-100"></div>
          <div className="grow bg-brand-200"></div>
          <div className="grow bg-brand-300"></div>
          <div className="grow bg-brand-400"></div>
          <div className="grow bg-brand-500"></div>
          <div className="grow bg-brand-600"></div>
          <div className="grow bg-brand-700"></div>
          <div className="grow bg-brand-800"></div>
          <div className="grow bg-brand-900"></div>
          <div className="grow bg-brand-950"></div>
        </div>
        <div className="flex grow flex-col basis-1/3">
          <div className="p-2 flex justify-between">
            <span>{over.name}</span>
            <span>{over.hue}</span>
          </div>
          <div className="grow bg-over-50"></div>
          <div className="grow bg-over-100"></div>
          <div className="grow bg-over-200"></div>
          <div className="grow bg-over-300"></div>
          <div className="grow bg-over-400"></div>
          <div className="grow bg-over-500"></div>
          <div className="grow bg-over-600"></div>
          <div className="grow bg-over-700"></div>
          <div className="grow bg-over-800"></div>
          <div className="grow bg-over-900"></div>
          <div className="grow bg-over-950"></div>
        </div>
      </div>
    </main>
  );
}
